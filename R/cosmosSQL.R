#' POST a custom query to the REST API for Cosmos DB.
#' 
#' @param sql SQL String for for querying database. Defaults to SELECT * FROM c
...
#' @keywords sql query cosmosdb post
#' @export
#' @examples
#' cosmosSQL(sql = "SELECT * FROM c", content.response = TRUE)

cosmosSQL <- function(sql = "SELECT * FROM c", max.items = 100, debug.auth = FALSE, debug.query = FALSE, content.response = FALSE) {
